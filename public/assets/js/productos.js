...
//                 }
//                 Swal.fire({
//                     html: `<h4><b>Error al editar el producto</b></h4><p>${errorMessages}</p>`,
//                     icon: `error`,
//                 });
//             },
//         });
...
function fn_editar_producto(id) {
    let settings = {
        ...
        "data": JSON.stringify({
            "categoryId": $(`#select_categories_${id}`).val(),
            "name": $(`#product_name_${id}`).val(),
            "type": $(`#select_type_${id}`).val(),
            "status": $(`#select_status_${id}`).val(),
            "featured": 0,
            "catalogVisibility": "visible",
            "description": $(`#description_${id}`).val(),
            "shortDescription": $(`#short_description_${id}`).val(),
            "regularPrice": $(`#regular_price_${id}`).val(),
            "salePrice": $(`#sale_price_${id}`).val(),
            "price": $(`#price_${id}`).val(),
            "onSale": 0,
            "stockQuantity": $(`#stock_quantity_${id}`).val(),
            "stockStatus": $(`#select_stock_status_${id}`).val(),
            "weight": "",
            "dimensions": "",
            "meta_data": null,
            "variation": 0,
